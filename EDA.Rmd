...
data_norm = as.data.frame(lapply(modified_data[1:39], normalize))
...
rdata_norm = as.data.frame(lapply(rf_data[1:39], normalize))
...
# predict_reg
# MAE(rdata_test$SalePrice, predict_reg)
# RMSE(rdata_test$SalePrice, predict_reg)
...
alpha0.fit <- cv.glmnet(x = data.matrix(data_train[,2:40]), y = data_train[,1], type.measure = "mse", alpha = 0, family= "gaussian")
alpha0.predicted <- predict(alpha0.fit, s=alpha0.fit$lambda.1se, newx= data.matrix(data_test[,2:40]))
# Lasso alpha = 1. similar equations, BUTT helps remove useless variables
alpha1.fit <- cv.glmnet(x = data.matrix(data_train[,2:40]), y = data_train[,1], type.measure = "mse", alpha = 1, family= "gaussian")
alpha1.predicted <- predict(alpha1.fit, s=alpha1.fit$lambda.1se, newx= data.matrix(data_test[,2:40]))
...
for (i in 0:10) {
  ...
  list.of.fits[[fit.name]] <- cv.glmnet(x = data.matrix(data_train[,2:40]), y = data_train[,1], type.measure="mae", alpha=i/10, family="gaussian")
}
results <- data.frame() # Database for the resulting error matrix
for (i in 0:10) {
  fit.name <- paste0("alpha", i/10)
  ## Use each model to predict 'y' given the Testing dataset
  predicted <- predict(list.of.fits[[fit.name]], s=list.of.fits[[fit.name]]$lambda.1se, newx=data.matrix(data_test[,2:40]))
  ## Error
...
# Lasso alpha = 1. similar equations, BUTT helps remove useless variables
ENR <- cv.glmnet(x = data.matrix(data_train[,2:40]), y = data_train[,1], type.measure = "mse", alpha = 0.6, family= "gaussian")
ENR.predicted <- predict(ENR, s=ENR$lambda.1se, newx= data.matrix(data_test[,2:40]))
```
...
noout_ENR <- cv.glmnet(x = data.matrix(data_train_noout[,2:40]), y = data_train_noout[,1], type.measure = "mse", alpha = 0.6, family= "gaussian")
END_pred <- predict(noout_ENR, s=noout_ENR$lambda.1se, newx= data.matrix(data_test_noout[,2:40]))
